          ...
          if(move_uploaded_file($this->request->data['file']['tmp_name'],$uploadFile)){
            ...
            for ($row = 2; $row <= $lastRow; $row++)
            {
              ...
              if ($resultDni->count() == 0)
              {
                  ...
                  $hoja[$row]['telefono1']  = $worksheet->getCell('F'.$row)->getValue();
                  $hoja[$row]['telefono2'] = $worksheet->getCell('G'.$row)->getValue();
                  $hoja[$row]['telefono3'] = $worksheet->getCell('H'.$row)->getValue();
                  $hoja[$row]['telefono4'] = $worksheet->getCell('I'.$row)->getValue();
                  ...
                  //cargo la deuda
                  ///deuda
                  $hoja[$row]['producto'] = $worksheet->getCell('J'.$row)->getValue();
                  $hoja[$row]['numero_prodcuto'] = $worksheet->getCell('K'.$row)->getValue();
                  $hoja[$row]['fecha_mora'] = $worksheet->getCell('L'.$row)->getFormattedValue();
                  $hoja[$row]['dias_mora'] = $worksheet->getCell('M'.$row)->getValue();
                  $hoja[$row]['capital_inicial'] = $worksheet->getCell('N'.$row)->getValue();
                  $hoja[$row]['total'] = $worksheet->getCell('O'.$row)->getValue();
                  ...
                  $connection->insert('deudas', [
                      ...
                      'producto' => $worksheet->getCell('J'.$row)->getValue(),
                      'numero_producto' => $worksheet->getCell('K'.$row)->getValue(),
                      'fecha_mora' => $fecha_mora,
                      'dias_mora' =>  $worksheet->getCell('M'.$row)->getValue(),
                      'capital_inicial' => $worksheet->getCell('N'.$row)->getValue(),
